...
stats ProdTensor::evalX(double *x) {
	...
	printf("===> dt = %f for constructing alignment graph\n",  timer.toc());
	...
	printf("===> dt = %f for counting triangles\n",  timer.toc());
...
alignment* ProdTensor::postprocess(double *x_final) {
/*
	this->n1 = this->G->n;
	this->n2 = this->H->n;
	this->n = n1*n2;  number of edges in L
*/
	...
	stats myStats = evalX(x_final);
	align->conserved_edges = myStats.M;
	align->conserved_triangles = myStats.T;

	align->mi = new int[myStats.num_matches];
	align->mj = new int[myStats.num_matches];
	align->PrefG = new vector<int>[myStats.num_matches];
	align->PrefH = new vector<int>[myStats.num_matches];
	for (i = 0; i < myStats.num_matches; i++) {
		align->mi[i] = (int)mi[i];
		align->mj[i] = (int)mj[i];
		align->PrefG[align->mj[i]].push_back(align->mi[i]);
		align->PrefH[align->mi[i]].push_back(align->mj[i]);
		temp_x[align->mi[i]*this->n2+align->mj[i]] = 0;
	}
...
*/
	...
	alignment* result = postprocess(best_x.memptr());
