...
for instance_id in {1..10}; do		
	...
	rm output/*
	./tri-match -t smat -H $H_net -G $G_net -S $SeqSim -Y 2 -b 1 -a 0 --iter 3 -x seqsim
	fname=`ls output/ -1t | head -1`
	outpath=/home/shahin/Dropbox/Projects/HONEA/experiment/output/NAPA/sorted/Family_$instance_id/cTAME_alpha=0_it=3
	mkdir -p $outpath
